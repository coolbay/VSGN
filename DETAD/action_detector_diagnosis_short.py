...
class ActionDetectorDiagnosis(object):
    ...
    def __init__(self, ground_truth_filename=None, prediction_filename=None,
        ...
        # self.prediction = self._import_prediction_txt(prediction_filename)
    ...
    def _import_prediction_txt(self, prediction_filename):
        labels = {}

        with open('../data/detclasslist.txt', 'r') as label_file:
            ff = label_file.readlines()
            for line in ff:
        ...
        video_lst, t_start_lst, t_end_lst = [], [], []
        label_lst, score_lst = [], []
        with open(prediction_filename, 'r') as infile:
            content = infile.readlines()
            for i, line in enumerate(content):
                videoid, t_start, t_end, label_idx, score = line.split()

                label = self.activity_index[labels[label_idx]]
                video_lst.append(videoid)
                t_start_lst.append(float(t_start))
                t_end_lst.append(float(t_end))
                label_lst.append(label)
    ...
    def wrapper_analyze_fp_error_types(self):
        ...
        fp_error_types_Error_list = Parallel(n_jobs=len(self.tiou_thresholds))(
        ...
        return fp_error_types_Error_list
    ...
    def diagnose(self):
        ...
        self.fp_error_types_Error_list = self.wrapper_analyze_fp_error_types()
        # self.fp_error_types, error_list = self.wrapper_analyze_fp_error_types()
        self.fp_error_types_count = {}


        # Modified by Chen for showing results for only short actions
        # -------------------------------------------------------------------------------------

        error_list_union = set()
        for tidx, column_name in enumerate(self.fp_error_type_cols):
            self.fp_error_types= self.fp_error_types_Error_list[tidx][0]
            error_list = self.fp_error_types_Error_list[tidx][1]
            self.prediction[column_name] = self.fp_error_types
            ...
            error_ids, counts = np.unique(self.fp_error_types, return_counts=True)
            ...
            error_list_union = set().union(list(error_list_union), error_list)

        self.prediction.drop(error_list_union, inplace=True)
        # -------------------------------------------------------------------------------------
...
def compute_average_precision_detection(ground_truth, prediction, tiou_thresholds=np.linspace(0.5, 0.95, 10),
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        try:
            ...
            # ground_truth_videoid = ground_truth_gbvn.get_group(this_pred['video-id'][2:])  # for TEM
            ground_truth_videoid = ground_truth_gbvn.get_group(this_pred['video-id'])   # for BSN
        except Exception as e:
            fp[:, idx] = 1
            continue
...
def analyze_fp_error_types(prediction,
    ...
    for idx, this_pred in this_prediction.iterrows():
        ...
        # By Chen for showing only the short actions
        # -----------------------------------------------------
        this_gt1 = this_gt.loc[(tiou_arr.argmax()) & (this_gt['coverage'] == 'XS')]

        if len(this_gt1) == 0:
            error_list.append(this_pred['prediction-id'])
            continue
        # -----------------------------------------------------
    ...
    # By Chen for showing only the short actions
    # -----------------------------------------------------
    # fp_error_types = np.delete(fp_error_types, error_list, 0)
    # -----------------------------------------------------
    
    return [fp_error_types, error_list]
