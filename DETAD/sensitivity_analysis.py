...
def plot_sensitivity_analysis(opt, sensitivity_analysis, save_filename,
    ...
    if opt['out_prop_map'] == 'true':
        plt.tight_layout()
        fig.savefig(save_filename,bbox_inches='tight')
        print('[Done] Output analysis is saved in %s' % save_filename)

def analyze(opt, ground_truth_filename, subset, prediction_filename, output_folder, is_thumos14):
    if not is_thumos14:
        if subset == 'testing':
            # ActivityNet testing
    ...
    plot_sensitivity_analysis(opt, sensitivity_analysis=sensitivity_analysis,
                              save_filename=output_folder,
                              colors=colors,
...
if __name__ == '__main__':
    ...
    analyze(args.ground_truth_filename, args.subset, args.prediction_filename, args.output_folder, args.is_thumos14)
